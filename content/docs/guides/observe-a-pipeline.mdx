...
tapstate exposes four observation faces. They answer different questions and should be read together with source and destination data.

| Face | Question | Storage boundary |
|---|---|---|
...
```text
status orders_to_analytics
status orders_to_analytics --watch
```
...
For a pipeline that uses `nest`, `dead_lettered` is the number of changes that
the nest could not place into a document, summed across its namespaces. A
nonzero value means those changes were retained in the dead-letter store rather
than assembled into output documents. Check the pipeline logs for the failure
code and affected key; a rising value means new unplaceable changes are still
arriving.
...
In `v0.2.0`, use target row counts and representative keyed samples as the primary snapshot proof. An empty `snapshot` result alone does not establish success or failure.

## Read and follow logs

```text
logs orders_to_analytics
logs orders_to_analytics --follow
```
...
- tapstate version and pipeline revision;
- final `status`;
