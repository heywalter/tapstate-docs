...
- **Quickstart:** no action is required. `quickstart.sh` downloads and
...
The `v0.3.0` preview documents MySQL and PostgreSQL as source artifacts and
...
In the interactive `v0.3.0` REPL, a failed line does not end the session. Inspect the report's `outcome`; do not treat the session's eventual exit as the connection-test result.
