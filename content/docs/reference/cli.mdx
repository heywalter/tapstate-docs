---
title: CLI reference
description: Use tapstate v0.3.0 offline workspace commands, connected sessions, and one-shot commands
ai:
  kind: reference
  id: cli
  maturity: preview
...
In `v0.3.0`, use `tapstate help <command>` or `tapstate <command> --help` to
...
In `v0.3.0`, row previews and live reads require a source that uses the
...
## Commands not implemented in v0.3.0

The top-level help lists `run`, `export`, `diff`, and `edit` as reserved verbs.
They return exit code `3` in `v0.3.0`. Do not build a workflow that depends on
them.
