...
| `artifact.reclaim-incomplete` | The deletion already took effect, but dependent bookkeeping left residue. Do not retry the deletion; inspect the `reason` and `residue` parameters and reconcile the remaining bookkeeping. A retry returns `artifact.not-found`. |
