...
description: Call the authenticated tapstate v0.2.1 control-plane endpoints and interpret preview responses
ai:
  kind: reference
  id: rest-api
  maturity: preview
...
The `v0.2.1` server exposes a preview HTTP control plane. This API exists only
...
| Method | Path | Boundary |
|---|---|---|
| `GET` | `/api/cluster/members` | Present but returns HTTP `501` in `v0.2.1` |
...
The v0.2.1 `GET /api/artifacts/{id}` read path currently returns HTTP `404`
...
- `v0.2.1` supports both interactive connected sessions and one-shot connected
