...
description: Call the authenticated tapstate v0.3.0 control-plane endpoints and interpret preview responses
ai:
  kind: reference
  id: rest-api
  maturity: preview
...
The `v0.3.0` server exposes a preview HTTP control plane. This API exists only
...
| Method | Path | Boundary |
|---|---|---|
| `GET` | `/api/cluster/members` | Present but returns HTTP `501` in `v0.3.0` |
...
The v0.3.0 `GET /api/artifacts/{id}` read path currently returns HTTP `404`
...
- `v0.3.0` supports both interactive connected sessions and one-shot connected
